...
class AsyncMcpServerWorker:
    ...
    async def start_server(self) -> None:
        ...
        if not ready:
            message = f"MCP 参考项目不存在或缺少 main.py/src：{root}"
    ...
    async def build_uvicorn_server(self):
        """按参考项目逻辑构建本机 Streamable HTTP MCP 服务。"""
        import uvicorn
        from starlette.requests import Request
        from starlette.responses import PlainTextResponse
        from starlette.routing import Route
        from wmpf_async_mcp.app.config import AppConfig
        from wmpf_async_mcp.app.service import WmpfAsyncService
        from wmpf_async_mcp.entrypoints.http_server import TokenMiddleware
        from wmpf_async_mcp.tools import create_mcp_app

        app_config = AppConfig.from_env()
        app_config.host = self.config.host
        app_config.port = int(self.config.port)
        app_config.token = self.config.token
        service = WmpfAsyncService(config=app_config)
    ...
    def reference_project_ready(root: Path) -> bool:
        """检查参考 MCP 项目是否包含入口和 src 目录。"""
        return (root / "main.py").exists() and (root / "src").is_dir()

    @staticmethod
    def prepare_reference_imports(root: Path) -> None:
        """把参考 MCP src 目录加入当前 worker 的 import 路径。"""

...
3. connection_ops(action="select_appservice_context")   ← 必须在 runtime_eval 之前！
   → 自动识别 appservice Runtime context
   → 若不先执行此步，后续 runtime_eval 会运行在 page-frame 上下文，
     导致 getCurrentPages / wx / __wxConfig 全部抛 ReferenceError

4. network_ops(action="hook_wx_request")
   → 在 appservice 上下文注入 wx.request hook，采集请求/响应原始数据

5. network_ops(action="hook_fetch_and_xhr")
   → 注入 fetch/XHR hook（Web 端 H5 页面）

6. runtime_ops(action="dump_runtime_snapshot")
...
runtime_ops(action="runtime_eval",
            expression="(()=>{const pages=getCurrentPages();return pages.map(p=> ({route:p.route,options:p.options,data:JSON.stringify(p.data).slice(0,200)}))})()")
  → 返回当前页面栈，每条记录包含 route 路径

# ⚠️ 上下文故障恢复：如果报 ReferenceError: getCurrentPages/wx/App is not defined
#    说明当前 eval 运行在 page-frame 而非 appservice。立即执行：
#    connection_ops(action="select_appservice_context") → 重试

# Step 2: 获取小程序完整路由表（从 __wxConfig 或 app.json）
debugger_ops(action="search_in_sources", query="router|pages|tabBar|subpackages")
  → 搜索路由配置

runtime_ops(action="runtime_eval",
            expression="(()=>{try{return JSON.stringify(__wxConfig?.page||{}).slice(0,3000)}catch(e){return 'N/A'}})()")
  → 获取页面配置及路径列表
```

### 1.1 逐路由流量采集（核心变更）

**对每个路由执行以下标准操作流，采集该路由产生的所有请求：**

```
For each route in [路由列表]:
  ┌─────────────────────────────────────────────────────┐
  │ 路由: pages/index/index                              │
  │   1. 导航/触发跳转到该路由                              │
  │      TabBar 页用 wx.switchTab，子页用 wx.navigateTo    │
  │   2. 等待页面加载完成 (2-3s)                            │
  │   3. network_ops(action="get_all_requests")           │
  │      → 筛选本路由产生的新请求                            │
  │   4. ⚡ 判断是否"空流量路由"（见下方决策树）              │
  │   5. 记录: 请求 URL / 方法 / 参数 / 加密特征            │
  │   6. 标注: 来源路由 = pages/index/index               │
  │   7. 对关键请求: get_request_detail(request_id)       │
  └─────────────────────────────────────────────────────┘
```
...
```
导航后 get_all_requests 返回 0 条新请求？
│
├─ YES → 页面可能已缓存，尝试强制刷新：
│    a. 调用 homePage.onPullDownRefresh() 或 onShow()
│    b. 如果仍无请求 → 导航到该路由的典型子页面
│       （如列表页 → 点进详情页 / 订单页 → 点进订单详情）
│    c. 如果子页面也无请求 → 判定为"静态路由/纯UI路由"
│       → 记录为"无接口 — 跳过"，立即进入下一条路由
│    d. ⚠️ 不要在空流量路由上反复重试超过 2 次，浪费时间
│
└─ NO → 正常采集，继续步骤 5-7
```

> **静态路由常见特征**：纯展示页（协议/关于/引导页）、已渲染完成的 TabBar 首页（切回时不重新请求）、仅读取 Storage 不发网络请求的配置页。识别后跳过即可，不要强行归因。

**路由请求标注格式（在分析和报告中均使用）：**

```
路由: pages/order/list (订单列表页)
  ├── GET  /api/order/list?page=1&size=20         [认证: Bearer token]
  │    加密: 无 / 响应加密 AES-CBC / 请求参数加密
  │    风险标签: 订单数据 / 分页可遍历
  ├── POST /api/user/recommend                     [认证: Bearer token]
  │    加密: 请求体有 mina_edata 参数
  │    风险标签: 用户数据
  └── GET  /api/config/app
       加密: 无
       风险标签: 配置泄露

路由: pages/terms/index (服务条款页)          ← 静态路由示例
  └── [无接口 — 纯展示，跳过]
```
